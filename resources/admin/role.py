...
from schemas.auth import UserSchema, UserRoleSchema , PagesSchema
...
blp = Blueprint("AdminRoles", "adminroles", description="Operations to Add Roles")
...
class AddPages(MethodView):
    ...
    def post(self):
        try:
            ...
            # Create New Roles
            roles = db.session.query(roleModel.UserRoleModel).filter_by(role_id=data['role_id']).first()

            if not roles:
                return jsonify({"status": False, "status_code": 404, "message": "Role not found", "data": {}}), 404
            
            for page in data['pages']:
                roles.pages.append(page)

            db.session.commit()

            return jsonify({"status": True, "status_code": 200, "message": "Pages Added Successfully", "data": {}}), 200
